...
public class GoldEye extends AbstractCanAliasModule {

    @Override
    public void setDemoState(EnumModuleState state) {
        try {
            KKPictureManager.getInstance(mContext).setAdvancedPQState(
                    KKPictureManager.ADVANCED_PQ_TYPE_GOLD_EYE, state.getValue());
        } catch (Throwable e) {
            e.printStackTrace();
        }
    }
    ...
    public EnumModuleState getDemoState() {
        ...
        try {
            int state = KKPictureManager.getInstance(mContext).getAdvancedPQState(
                    KKPictureManager.ADVANCED_PQ_TYPE_GOLD_EYE);
    ...
    public boolean isSupport() {
        ...
        try {
            ret = KKConfigManager.getInstance(mContext).getBooleanConfig(
                    KKConfigManager.EN_KK_SYSTEM_CONFIG_KEY_BOOLEAN.SUPPORT_GOLDEN_EYE);
        } catch (Throwable e) {
            e.printStackTrace();
        }
        return ret;
