...
public class LocalDimming extends AbstractCanAliasModule {

    @Override
    public void setDemoState(EnumModuleState state) {
        try {
            KKPictureManager.getInstance(mContext).setAdvancedPQState(
                    KKPictureManager.ADVANCED_PQ_TYPE_LOCAL_DIMMING, state.getValue());
        } catch (Throwable e) {
            e.printStackTrace();
        }
    }
    ...
    public EnumModuleState getDemoState() {
        ...
        try {
            int state = KKPictureManager.getInstance(mContext).getAdvancedPQState(
                    KKPictureManager.ADVANCED_PQ_TYPE_LOCAL_DIMMING);
    ...
    public boolean isSupport() {
        ...
        try {
            ret = KKConfigManager.getInstance(mContext).getBooleanConfig(
                    KKConfigManager.EN_KK_SYSTEM_CONFIG_KEY_BOOLEAN.SUPPORT_LOCAL_DIMING);
        } catch (Throwable e) {
            e.printStackTrace();
        }
        return ret;
